...
static void orbisKeyboardEventUpdate(void *arg,const OrbisKeyboardEvent *e)
{
	...
	switch(e->id)
	{
		case ORBIS_KEYBOARD_EVENT_OPEN:
        	debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard opened for userId:0x%08x\n",e->param.resourceIdArray.userId);
			for(i=0;i<3;i++)
			{
        		debugNetPrintf(DEBUGNET_DEBUG,"%d ",e->param.resourceIdArray.resourceId[i]);
			}
		...
		debugNetPrintf(DEBUGNET_DEBUG, "liborbiskeyboard key down for keycode:0x%04X\n",e->param.keycode.keycode);
		debugNetPrintf(DEBUGNET_DEBUG, "liborbiskeyboard key down for status:%8x\n",e->param.keycode.status);
		//debugNetPrintf(DEBUGNET_DEBUG, "liborbiskeyboard key down for unknown1:%8x\n",e->param.keycode.unknown1);
		//debugNetPrintf(DEBUGNET_DEBUG, "liborbiskeyboard key down for userId:%8x\n",e->param.keycode.userId);
		//debugNetPrintf(DEBUGNET_DEBUG, "liborbiskeyboard key down for resourceId:%8x\n",e->param.keycode.resourceId);
		//debugNetPrintf(DEBUGNET_DEBUG, "liborbiskeyboard key down for unknown2:%8x\n",e->param.keycode.unknown2);
		
		//debugNetPrintf(DEBUGNET_DEBUG, "liborbiskeyboard key down for timestamp:%lu\n",e->param.keycode.timestamp);
		debugNetPrintf(DEBUGNET_DEBUG, "liborbiskeyboard key down for caps %d ctrll %d ctrlr %d shiftl %d shiftr %d altl %d altr %d cmdl %d cmdr %d\n",orbisKeyboardConf->caps_key,orbisKeyboardConf->control_key_left,orbisKeyboardConf->control_key_right,orbisKeyboardConf->shift_key_left,orbisKeyboardConf->shift_key_right,orbisKeyboardConf->alt_key_left,orbisKeyboardConf->alt_key_right,orbisKeyboardConf->cmd_key_left,orbisKeyboardConf->cmd_key_right);
		//		if((e->param.keycode.status&0x00000003)==0x00000003)

		if((e->param.keycode.status&0x00000003))
		{
		debugNetPrintf(DEBUGNET_DEBUG, "liborbiskeyboard key down :%02X\n",orbisKeyboardNames[index][e->param.keycode.keycode]);
		...
		case ORBIS_KEYBOARD_EVENT_KEYCODE_UP:
			debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard key up for keycode:0x%04x\n",e->param.keycode.keycode);
			orbisKeybordGetSpecialKeys(e);
			if(e->param.keycode.keycode<0x001e || e->param.keycode.keycode==0x0033)
			{
				index=orbisKeyboardConf->caps_key|orbisKeyboardConf->shift_key_left|orbisKeyboardConf->shift_key_right;
		...
		case ORBIS_KEYBOARD_EVENT_KEYCODE_REPEAT:
			debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard key repeat for keycode:0x%04x\n",e->param.keycode.keycode);
			break;
		case ORBIS_KEYBOARD_EVENT_CONNECTION:
    		debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard connected for userId:0x%08x\n",e->param.resourceIdArray.userId);
			for(i=0;i<3;i++)
			{
    			debugNetPrintf(DEBUGNET_DEBUG,"%d ",e->param.resourceIdArray.resourceId[i]);
			}
    	...
    	debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard disconnected for userId:0x%08x\n",e->param.resourceIdArray.userId);
		for(i=0;i<3;i++)
		{
    		debugNetPrintf(DEBUGNET_DEBUG,"%d ",e->param.resourceIdArray.resourceId[i]);
		}
    	...
    	debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard aborted\n");
		
			break;
		default:
    	debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard unknown event\n");
...
void orbisKeyboardClose()
{
	...
	if(orbisKeyboardConf!=NULL)
	{
		...
		if(orbisKeyboardConf->status==0 )
		{
			...
			if(ret<0)
			{
		        debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard sceImeKeyboardClose return error 0x%8x\n",ret);
...
{
    debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard sceImeKeyboardOpen in\n");
	sleep(1);
	OrbisKeyboardParam param;
	int ret;
	if(orbisKeyboardConf!=NULL)
	{
	    debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard orbisKeyboardConf not null \n");
		
		if(orbisKeyboardConf->orbiskeyboard_initialized!=1  )
		{
		    debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard orbisKeyboardConf %d \n",orbisKeyboardConf->orbiskeyboard_initialized);
		...
		if(ret<0)
		{
	        debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard sceImeKeyboardOpen return error 0x%8x\n",ret);
	        return -1;
		}
		sleep(1);
        debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard sceImeKeyboardOpen return success %d\n",ret);
		orbisKeyboardConf->status=0; //opened
		return 0;
	}
	else
	{
        debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard orbisKeyboardOpen error config null\n");
...
void orbisKeyboardFinish()
{
	if(orbiskeyboard_external_conf!=1)
	{
		if(orbisKeyboardConf!=NULL)
		{
			...
			orbisKeyboardConf->userId=0;
			orbisKeyboardConf->orbiskeyboard_initialized=-1;
			debugNetPrintf(DEBUGNET_DEBUG,"liborbisKeyboard finished\n");
...
int orbisKeyboardInitWithConf(OrbisKeyboardConfig *conf)
{
	...
	if(ret)
	{
		debugNetPrintf(DEBUGNET_DEBUG,"liborbisKeyboard already initialized using configuration external\n");
		debugNetPrintf(DEBUGNET_DEBUG,"orbiskeyboard_initialized=%d\n",orbisKeyboardConf->orbiskeyboard_initialized);
		debugNetPrintf(DEBUGNET_DEBUG,"ready to have a lot of fun :P ...\n");
		return orbisKeyboardConf->orbiskeyboard_initialized;
	}
	else
	{
...
int orbisKeyboardInit()
{
	...
	if (orbisKeyboardConf->orbiskeyboard_initialized==1) 
	{
		debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard is already initialized!\n");
		return orbisKeyboardConf->orbiskeyboard_initialized;
	}
	...
	if(ret<0)
	{
        debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard sceSysmoduleLoadModule return error 0x%8x\n",ret);
        return -1;
	}
	...
	if(ret<0)
	{
        debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard sceUserServiceGetInitialUser return error 0x%8x\n",ret);
        return -1;
	...
	debugNetPrintf(DEBUGNET_DEBUG,"liborbiskeyboard is initialized!\n");
